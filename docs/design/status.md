...
>
> **Verbleibend sind drei Punkte, keiner davon blockierend.** Zwei Engine-Defekte sind formuliert und werden von `tests/engine_defects_test.php` festgehalten: der ungeprüfte `array_key_first`-Aufruf (catquiz#59) und der ungeprüfte `lastquestion`-Zugriff, der `store_debug_info` bei `DEBUG_DEVELOPER` unbenutzbar macht und damit die θ-Trajektorie kostet. Drittens braucht es für publikationsreife Aussagen Replikationszahlen deutlich über fünf — bei 30 Attempts je Zelle überlappen die Konfidenzintervalle der Poolvarianten noch fast vollständig.
